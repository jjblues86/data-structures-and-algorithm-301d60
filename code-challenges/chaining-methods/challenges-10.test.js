...
const count = (target, input) => {
  ...
  return input.reduce((acc, ele) => {
    return acc + ele.filter(num => num === target).length;
  }, 0);
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(acc => acc.gender === 'male' || acc.gender === 'female').map(acc => acc.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((acc, ele) => acc.height < ele.height ? ele:acc).name;
};
