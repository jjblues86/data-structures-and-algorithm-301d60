...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((accumulator, element) => accumulator.concat([element.name]), []);
};
...
const reversedString = (str) => {
  // Solution code here...
  return str.split('').reduce((accumulator, element) => element + accumulator);
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((accumulator, element) => {
    if(element.children){
      accumulator = accumulator + element.children.length;
    }
    return accumulator;
  }, 0);
};
