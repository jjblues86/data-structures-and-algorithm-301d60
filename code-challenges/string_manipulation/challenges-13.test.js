...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(ele => ele.split('').filter(str => /\d/gm.test(str)).join(''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((ele, idx) => idx % 2 === 1).join('');
};
...
const findEvery = (arr, target) => {
  ...
  let every = true;
  arr.map(str => {
    if(!str.includes(target)) every = false
  });
  return every;
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((str, ele) => str.charAt(ele));
};
